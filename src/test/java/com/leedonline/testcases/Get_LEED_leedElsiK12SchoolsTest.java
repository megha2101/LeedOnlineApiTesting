...
public class Get_LEED_leedElsiK12SchoolsTest extends BaseClass{
	@Test
	@Parameters({"rowNum", "SheetName" })
	public void Get_LEED_leedElsiK12Schools(int rowNum, String SheetName) throws IOException {
		try {	
			...
			CommonMethod.res = given().log().ifValidationFails()
					...
					.spec(reqSpec)
					.params("q", data.getCellData(SheetName, "q", rowNum))
					.when()
					.get("/LEED/elsiK12Schools")
					.then()
					.extract()
					.response();
			...
			CommonMethod.test =  CommonMethod.extent
					.startTest("Get_LEED_leedElsiK12SchoolsTest Api "+ CommonMethod.getLabel(CommonMethod.responseTimeInMS()),
							"Get ELSI K12 Schools for autocomplete.")
			...
			CommonMethod.checkForSpecificSubListHashmapKeyValue("result", "id", "school");
