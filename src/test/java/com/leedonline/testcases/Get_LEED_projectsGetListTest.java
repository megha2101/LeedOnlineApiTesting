...
public class Get_LEED_projectsGetListTest extends BaseClass{
	@Test
	@Parameters({"rowNum", "SheetName" })
	public void Get_LEED_projectsGetList(int rowNum, String SheetName) throws IOException {
		try {	
			...
			CommonMethod.res = given().log().ifValidationFails()
					...
					.spec(reqSpec)
					.when()
					.get("/Projects/getList/"+data.getCellData(SheetName, "projectType", rowNum))
					.then()
					.extract()
					.response();
			...
			CommonMethod.test =  CommonMethod.extent
					.startTest("Get_LEED_projectsGetList Api "+ CommonMethod.getLabel(CommonMethod.responseTimeInMS()),
							"Get projects list.")
			...
			CommonMethod.checkForSpecificSubHashMapKeyValue("filters", "type", "total");
			CommonMethod.checkforSpecificKeyValue("npk", "lastSync");
			CommonMethod.checkForSpecificSubListHashmapKeyValue("result", "id", "status");
