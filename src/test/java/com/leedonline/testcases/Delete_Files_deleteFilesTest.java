...
public class Delete_Files_deleteFilesTest extends BaseClass{
	@Test
	@Parameters({"rowNum", "SheetName" })
	public void Delete_Files_deleteFiles(int rowNum, String SheetName) throws IOException {
		try {	
			...
			CommonMethod.res = given().log().ifValidationFails()
					...
					.spec(reqSpec)
					.param("ids", data.getCellData(SheetName, "deleteFileId", rowNum))
					.when()
					.delete("/Files/delete")
					.then()
					.extract()
			...
			CommonMethod.test =  CommonMethod.extent
					.startTest("Delete_Files_deleteFiles Api "+ CommonMethod.getLabel(CommonMethod.responseTimeInMS()),
							"Delete single or multiple files from a project.")
			...
			CommonMethod.checkforRespContainsValue("deleted");
			CommonMethod.checkForSpecificSubHashMapKeyValue("result", data.getCellData(SheetName, "deleteFileId", rowNum), data.getCellData(SheetName, "deleteFileId", rowNum));
