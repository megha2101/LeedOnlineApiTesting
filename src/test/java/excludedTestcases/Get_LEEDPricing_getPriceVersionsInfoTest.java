...
public class Get_LEEDPricing_getPriceVersionsInfoTest extends BaseClass{
	@Test
	@Parameters({"rowNum", "SheetName" })
	public void Get_LEEDPricing_getPriceVersionsInfo(int rowNum, String SheetName) throws IOException {
		try {	
			...
			CommonMethod.res = given().log().ifValidationFails()
					...
					.spec(reqSpec)
					.param("versions",data.getCellData(SheetName, "versions", rowNum))
					.when()
					.get("/LEEDPricing/getPriceVersionsInfo")
					.then()
					.extract()
					.response();
			...
			CommonMethod.test =  CommonMethod.extent
					.startTest("Get_LEEDPricing_getPriceVersionsInfo Api "+ CommonMethod.getLabel(CommonMethod.responsetime),
